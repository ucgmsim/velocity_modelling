...
class TomographyData:
    def __init__(self, cvm_registry: CVMRegistry, tomo_name: str, offshore_surface_name: str, offshore_v1d_name: str, logger: Logger = None):
        ...
        offshore_surface_path = cvm_registry.get_info('surface', offshore_surface_name)['path']
...
class BasinData:
    def __init__(self, cvm_registry: CVMRegistry, basin_name: str, logger: Logger = None):
        ...
        self.surf = [cvm_registry.load_basin_surface(surface['path']) for surface in basin_info['surfaces']]
...
class CVMRegistry:
    ...
    def get_full_path(self, relative_path: Path) -> Path:
        ...
        Path
            The full path.
        """
        return DATA_ROOT / relative_path if not Path(relative_path).is_absolute() else Path(relative_path)
    ...
    def load_tomo_surface_data(self, tomo_name: str, offshore_surface_name: str = DEFAULT_OFFSHORE_DISTANCE,
                               offshore_v1d_name: str = DEFAULT_OFFSHORE_1D_MODEL) -> TomographyData:
        """
    ...
    def load_smooth_boundaries(self, nz_tomography_data: TomographyData, basin_names: List[str]):
        """
