...
import random
import os
from velocity_modelling.cvm.scripts.compare_emod3d import compare_output_files, parse_nzvm_config
...
C_BINARY_DIR = Path("/home/seb56/Velocity-Model")
...
def generate_random_nzvm_config(c_output_dir: Path) -> Path:
    ...
    topo_type = random.choice(["BULLDOZED","SQUASHED","SQUASHED_TAPERED","TRUE"])  # Fixed to a known valid value
...
def test_c_vs_python_velocity_model(setup_test_environment):
    ...
    c_result = subprocess.run(
        [
            "./NZVM",
            "nzvm.cfg"  # Relative path since we're in C_BINARY_DIR
        ],
    ...
    assert c_result.returncode == 0, f"C binary failed with return code {c_result.returncode}: {c_result.stderr} (stdout: {c_result.stdout})"
    
    # Check C binary output
    c_velocity_model_dir = c_output_dir / "Velocity_Model"
    assert c_velocity_model_dir.exists() and any(c_velocity_model_dir.iterdir()), f"No output found in {c_velocity_model_dir}"
