                        ...
                        script {
                            ...
                            withEnv(["JENKINS_OUTPUT_DIR=${test_output_dir}"]) {
                                sh """
                                    ...
                                    # test_gen_3dvm_c_vs_python.py and test_gen_3dvm_scenarios.py compare
                                    # against reference data generated by the
                                    # nzvm C binary. That binary does not
                                    # include the EMOD3D padding row (see commit
                                    # "add padding row for e3d.par") and samples
                                    # according to our incorrect idea of how the
                                    # velocity model should sample, so its
                                    # output can no longer match ours and these
                                    # tests are excluded here.
                                    pytest -s tests/ --benchmark-dir /nzvm/benchmarks --nzvm-binary-path /nzvm/NZVM --data-root ${env.WORKSPACE}/velocity_modelling/nzcvm_data --ignore=tests/test_gen_3dvm_c_vs_python.py --ignore=tests/test_gen_3dvm_scenarios.py
                                """
                            }
                        }
                    }
